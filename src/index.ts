 ...
 * ```
 */
export async function getRepos(username: string, token?: string): Promise<RepoMetadata[]> {
  // Input validation
  ...
  const reposRes: Response = await fetchWithRateLimit(
  ...
  for (const repo of repos) {
    try {
      const configRes = await fetchWithRateLimit(
        `https://api.github.com/repos/${cleanUsername}/${repo.name}/contents/src/repo.config.json`,
        { headers }
      );
