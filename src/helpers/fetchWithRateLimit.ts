  ...
  return limiter.schedule(async () => {
    ...
    if (res.status === 403 && remaining === "0") {
      if (retryCount >= maxRetries) {
        throw new Error(
          `Rate limit exceeded. Maximum retries (${maxRetries}) reached.`
        );
      }

      const resetTime = reset ? parseInt(reset, 10) * 1000 : Date.now() + 60_000;
      const waitTime = Math.max(resetTime - Date.now(), 1000);

      console.warn(
        `Rate limit hit. Waiting ${Math.ceil(waitTime / 1000)}s before retry...`
      );
    ...
    if (res.status >= 500 && res.status < 600 && retryCount < maxRetries) {
      ...
      console.warn(
        `Server error ${res.status}. Retrying in ${delay}ms (attempt ${
          retryCount + 1
        }/${maxRetries})`
      );
